...
int main(int argc, char const *argv[]) {
	...
	int q, i, j, k, count, prevision, reality;
	FILE* fin;
	...
	//	print_(aL, 10);
	    ...
	    for (int i = 0; i < no_out_nodes; ++i) {
	    	delta_L[i] = aL[i] - labels_train[k][i];
	...
	//	print_(delta_L, 10);
	    for (int i = 0; i < no_hidden; ++i) {
	    	...
	    	for (int j = 0; j < no_in_nodes; ++j) {
	    		err_hidm[j][i] = delta_1[i] * train[k][j];
	    	}
	    }
	//	print_(delta_1, 100);  
	    for (int i = 0; i < no_in_nodes; ++i) for (int j = 0; j < no_hidden; ++j) {
			hid_mat[i][j] -= learning_rate * err_hidm[i][j];
		}
		for (int i = 0; i < no_hidden; ++i) { b1[i] -= learning_rate * err_b1[i]; }
	
		for (int i = 0; i < no_hidden; ++i) for (int j = 0; j < no_out_nodes; ++j) {
			last_mat[i][j] -= learning_rate * err_last[i][j];
		}
		for (int i = 0; i < no_out_nodes; ++i) { bL[i] -= learning_rate * err_bL[i]; }
	}
...
int max_argument(double aL[no_out_nodes]) {
	int position = 0;
	for (int i = 1; i < no_out_nodes; ++i) if (aL[i] >= aL[position]) { position = i; }
	return position; }
